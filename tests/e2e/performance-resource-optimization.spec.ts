...
async function getResourceMetrics(page: Page): Promise<ResourceMetrics> {
	...
	return await page.evaluate(() => {
		...
		const cssResources = resources.filter(r => 
			r.name.includes('.css') || r.name.includes('/_next/static/css/'));
		...
		return {
			...
			cacheHitRate: resources.length > 0 ? (cachedResources.length / resources.length) * 100 : 0,
...
// 画像最適化メトリクスを取得
async function getImageOptimizationMetrics(page: Page): Promise<ImageOptimizationMetrics> {
	return await page.evaluate(() => {
		const resources = performance.getEntriesByType('resource') as PerformanceResourceTiming[];
		const imageResources = resources.filter(r => 
			r.initiatorType === 'img' || 
		...
		// 画像フォーマット別カウント
		const webpImages = imageResources.filter(r => r.name.includes('.webp')).length;
		const avifImages = imageResources.filter(r => r.name.includes('.avif')).length;
		const jpegImages = imageResources.filter(r => r.name.match(/\.(jpg|jpeg)$/i)).length;
		const pngImages = imageResources.filter(r => r.name.includes('.png')).length;

		// 最適化された画像の比率（WebP + AVIF）
		const optimizedImages = webpImages + avifImages;
		const optimizedRatio = imageResources.length > 0 
		...
		const avgImageSize = imageResources.length > 0 
			? totalImageSize / imageResources.length 
			: 0;
	...
	test("画像フォーマット最適化確認", async ({ page }) => {
		...
		if (imageMetrics.totalImages > 0) {
			...
			if (imageMetrics.optimizedRatio < 30) {
				console.warn(`⚠️ 画像最適化率が低いです: ${imageMetrics.optimizedRatio.toFixed(1)}%`);
				console.warn(`📝 Next.jsのImage componentを使用して自動最適化を活用してください`);
			}
		} else {
	...
	test("Next.js Image最適化設定確認", async ({ page }) => {
		...
		if (totalImageCount > 0) {
			...
			if (nextImageUsageRate < 50) {
				console.warn(`📝 推奨: より多くの画像でNext.js Imageコンポーネントを使用してください`);
			}
		}
	});
});
...
test.describe("ネットワーク・キャッシュ最適化", () => {
	...
	test("キャッシュ効率確認", async ({ page }) => {
		...
		if (secondLoadMetrics.totalResources > 5) {
			...
			expect(secondLoadMetrics.cacheHitRate).toBeGreaterThan(firstLoadMetrics.cacheHitRate);
		}

		// 静的リソースのキャッシュ効率: 30%以上（開発環境では柔軟に）
		if (secondLoadMetrics.cacheHitRate < 30) {
			console.warn(`⚠️ キャッシュヒット率が低いです: ${secondLoadMetrics.cacheHitRate.toFixed(1)}%`);
			console.warn(`📝 本番環境では適切なCache-Controlヘッダーの設定を確認してください`);
		}
	});
	...
	test("HTTP/2 multiplexing 効果確認", async ({ page }) => {
		...
		const navigationTiming = await page.evaluate(() => {
			...
			// NaN値を防ぐための安全な計算
			const domContentLoaded = nav && nav.domContentLoadedEventEnd && nav.navigationStart
				? nav.domContentLoadedEventEnd - nav.navigationStart
				: 0;
			const loadComplete = nav && nav.loadEventEnd && nav.navigationStart
			...
			return {
				...
				maxResourceDuration: resources.length > 0 ? Math.max(...resources.map(r => r.duration)) : 0,
				avgResourceDuration: resources.length > 0 ? resources.reduce((sum, r) => sum + r.duration, 0) / resources.length : 0
			};
		});
		...
		if (navigationTiming.domContentLoaded > 0) {
			...
			console.warn('⚠️ DOMContentLoaded値が取得できませんでした（Navigation Timing API制限）');
		}
		...
		if (navigationTiming.loadComplete > 0) {
			...
			console.warn('⚠️ LoadComplete値が取得できませんでした（Navigation Timing API制限）');
		}
		
		// 並列読み込みの効果確認（値が取得できた場合のみ）
		if (navigationTiming.loadComplete > 0 && navigationTiming.avgResourceDuration > 0) {
			const theoreticalSequentialTime = navigationTiming.avgResourceDuration * navigationTiming.resourceCount;
			const parallelEfficiency = navigationTiming.loadComplete / theoreticalSequentialTime;
			
			console.log(`🔄 並列効率: ${(parallelEfficiency * 100).toFixed(1)}% (数値が小さいほど並列効果が高い)`);
...
test.describe("リソース最適化総合評価", () => {
	test("全体的なリソース効率評価", async ({ page }) => {
		...
		for (const url of pages) {
			...
			// JavaScript サイズペナルティ
			if (metrics.jsSize > 2 * 1024 * 1024) score -= 20; // 2MB超過
			else if (metrics.jsSize > 1.5 * 1024 * 1024) score -= 10; // 1.5MB超過
			
			// CSS サイズペナルティ
			if (metrics.cssSize > 60 * 1024) score -= 15; // 60KB超過
		...
		for (const result of results) {
			console.log(`${result.page}: ${result.score}点 (JS: ${(result.metrics.jsSize/1024).toFixed(0)}KB, CSS: ${(result.metrics.cssSize/1024).toFixed(0)}KB, 応答: ${result.metrics.avgResponseTime.toFixed(0)}ms)`);
		}
