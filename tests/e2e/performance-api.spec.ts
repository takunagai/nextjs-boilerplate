...
// CSRFトークンを取得する共通関数
async function getCsrfToken(request: APIRequestContext): Promise<{ token: string; cookies: string }> {
...
async function measureConcurrentRequests(
	...
	for (let i = 0; i < concurrency; i++) {
		const promise = (async () => {
			...
			try {
				const response = requestOptions.method === 'GET'
...
test.describe("REST API パフォーマンス", () => {
	...
	test("User Registration API - バリデーションエラー性能", async ({ request }) => {
		// CSRFトークン取得
		const { token, cookies } = await getCsrfToken(request);
...
test.describe("Server Actions パフォーマンス", () => {
	test("Contact Form Server Action - 応答時間測定", async ({ page }) => {
		...
		for (let i = 0; i < iterations; i++) {
			...
			await page.locator('textarea[name="message"], textarea[id*="message"]').first().fill(
			...
			try {
				await page.waitForSelector('[data-testid="success-message"], .success', { 
					timeout: 3000 
				});
			} catch {
...
test.describe("並列処理・負荷テスト", () => {
	test("CSRF Token API - 並列リクエスト処理", async ({ request }) => {
		...
		console.log(`🚀 CSRF Token 並列性能: ${result.requestsPerSecond.toFixed(1)} req/sec`);
	});
	...
	test("User Registration API - 並列処理耐性", async ({ request }) => {
		...
		console.log(`🚀 Registration 並列性能: ${result.requestsPerSecond.toFixed(1)} req/sec`);
	});
});
...
test.describe("エラー耐性・セキュリティ性能", () => {
	test("CSRF検証失敗時の応答性能", async ({ request }) => {
		...
		console.log(`🔒 CSRF検証失敗: 平均${metrics.avgResponseTime.toFixed(1)}ms で高速拒否`);
	});
	...
	test("レート制限機能との協調確認", async ({ request }) => {
		...
		if (rateLimitedRequests > 0) {
			console.log(`✅ レート制限が正常に機能: ${rateLimitedRequests}件のリクエストを制限`);
		}
...
test.describe("APIパフォーマンス総合評価", () => {
	test("全API総合パフォーマンススコア", async ({ request, page }) => {
		...
		// 1. CSRF Token API
		const csrfMetrics = await measureApiPerformance(request, "/api/csrf-token", {
			method: 'GET',
			iterations: 3,
		...
		scores.push({ api: "CSRF Token", score: Math.max(0, csrfScore), responseTime: csrfMetrics.avgResponseTime });
		
		// 2. User Registration API
		try {
			const { token, cookies } = await getCsrfToken(request);
			const regMetrics = await measureApiPerformance(request, "/api/auth/register", {
				method: 'POST',
				data: { name: "評価用", email: "eval@test.com", password: "password123" },
				headers: {
					'Content-Type': 'application/json',
					'X-CSRF-Token': token,
					'Cookie': cookies,
			...
			scores.push({ api: "User Registration", score: Math.max(0, regScore), responseTime: regMetrics.avgResponseTime });
		} catch (error) {
		...
		for (const score of scores) {
			console.log(`${score.api}: ${score.score}点 (応答時間: ${score.responseTime.toFixed(1)}ms)`);
		}
