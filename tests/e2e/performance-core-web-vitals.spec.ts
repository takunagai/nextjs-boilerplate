...
async function injectWebVitalsScript(page: Page): Promise<void> {
	await page.addInitScript(() => {
		...
		if ("PerformanceObserver" in window) {
			...
			try {
				new PerformanceObserver((entryList) => {
					const fcpEntry = entryList.getEntriesByName(
		...
		window.addEventListener("load", () => {
			...
			if (navigation) {
				(window as any).__webVitals.ttfb =
...
async function measurePagePerformance(
	...
	console.log(
		`⏱️ ナビゲーション総時間: ${navigationEndTime - navigationStartTime}ms`,
	);
	console.log("📈 測定結果:", {
...
test.describe("Core Web Vitals パフォーマンス測定", () => {
	...
	test("お問い合わせページ (/contact) - フォーム応答性確認", async ({
		page,
	}) => {
		const requirements: PerformanceRequirements = {
	...
	test("ダッシュボードページ (/dashboard) - 認証後パフォーマンス", async ({
		page,
	}) => {
