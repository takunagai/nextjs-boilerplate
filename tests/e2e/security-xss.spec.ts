...
class SecurityTestPage {
	...
	async fillContactFormWithPayload(payload: string) {
		...
		await this.page.getByLabel("メールアドレス").fill(`test${Math.random()}@example.com`);
	...
	async checkNoScriptExecution() {
		...
		expect(pageContent).not.toMatch(/<iframe[^>]*src\s*=\s*['"]javascript\s*:\s*alert/i);
...
test.describe("XSS脆弱性テスト", () => {
	...
	test.describe("フォーム入力XSSテスト", () => {
		XSS_PAYLOADS.forEach((payload, index) => {
			test(`お問い合わせフォーム: XSSペイロード ${index + 1} の無害化`, async ({ page }) => {
				...
				const errorElements = page.locator('.text-destructive, .error, [role="alert"]');
		...
		XSS_PAYLOADS.slice(0, 3).forEach((payload, index) => {
			test(`ログインフォーム: XSSペイロード ${index + 1} の無害化`, async ({ page }) => {
	...
	test.describe("URLパラメータXSSテスト", () => {
		...
		urlPayloads.forEach((payload, index) => {
			test(`URLパラメータXSS ${index + 1}: クエリパラメータの無害化`, async ({ page }) => {
	...
	test.describe("コンテンツタイプ検証", () => {
		...
		test("CSP（Content Security Policy）の存在確認", async ({ page }) => {
			...
			if (!hasCspHeader && cspMeta === 0) {
				console.warn("CSP (Content Security Policy) が設定されていません。セキュリティ強化を検討してください。");
			}
