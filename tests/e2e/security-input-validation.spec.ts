...
class InputValidationTestPage {
	...
	async expectValidationErrors() {
		const errorElements = await this.page.locator(".text-destructive, .error, [role='alert']").count();
...
test.describe("入力検証・サニタイゼーション セキュリティテスト", () => {
	...
	test.describe("基本バリデーション", () => {
		...
		test("メールアドレス形式の検証", async ({ page }) => {
			...
			for (const invalidEmail of INVALID_EMAILS.slice(0, 5)) { // 最初の5件をテスト
				await validationPage.fillAndSubmitContact({
					name: "テスト名前",
					email: invalidEmail,
					message: "テストメッセージです。",
	...
	test.describe("HTMLインジェクション対策", () => {
		DANGEROUS_INPUTS.html.forEach((htmlInput, index) => {
			test(`HTMLインジェクション ${index + 1}: ${htmlInput.substring(0, 30)}...`, async ({ page }) => {
				await validationPage.gotoContact();
				...
				expect(pageContent).not.toMatch(new RegExp(htmlInput.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'));
			});
		});
	...
	test.describe("SQLインジェクション対策", () => {
		DANGEROUS_INPUTS.sql.forEach((sqlInput, index) => {
			test(`SQLインジェクション ${index + 1}: ${sqlInput}`, async ({ page }) => {
				await validationPage.gotoContact();
				...
				expect(pageText || "").not.toMatch(/database error|sql error|mysql|postgresql/i);
			});
		});
		...
		test("登録フォームでのSQLインジェクション対策", async ({ page }) => {
			...
			await page.getByLabel("メールアドレス").fill("admin'; DROP TABLE users; --");
	...
	test.describe("制御文字・特殊文字対策", () => {
		DANGEROUS_INPUTS.control.forEach((controlInput, index) => {
			test(`制御文字・特殊文字 ${index + 1}: ${controlInput.replace(/[\x00-\x1F]/g, '\\x' + '$&'.charCodeAt(0).toString(16).padStart(2, '0'))}`, async ({ page }) => {
				await validationPage.gotoContact();
	...
	test.describe("長大入力攻撃対策", () => {
		test("非常に長い文字列の処理", async ({ page }) => {
			...
			const responseTime = await validationPage.measureResponseTime(async () => {
		...
		test("長大なHTMLタグ攻撃の防止", async ({ page }) => {
			...
			const longHtmlAttack = "<script>" + "a".repeat(5000) + "alert(1)" + "a".repeat(5000) + "</script>";
	...
	test.describe("エンコーディング攻撃対策", () => {
		DANGEROUS_INPUTS.encoding.forEach((encodedInput, index) => {
			test(`エンコーディング攻撃 ${index + 1}: ${encodedInput}`, async ({ page }) => {
				await validationPage.gotoContact();
	...
	test.describe("レスポンス時間攻撃対策", () => {
		...
		test("パスワード検証のタイミング攻撃対策", async ({ page }) => {
			...
			const passwords = ["short", "averagelength", "verylongpasswordthatexceedstypicallimits"];
	...
	test.describe("CSP（Content Security Policy）相互作用", () => {
		test("CSPヘッダーとの相互作用でインラインスクリプト実行防止", async ({ page }) => {
			await validationPage.gotoContact();
			...
			await page.waitForTimeout(1000);
			const cspErrors = logs.filter(log => log.includes('Content Security Policy'));
