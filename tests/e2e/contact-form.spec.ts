...
test.describe("お問い合わせフォームのテスト", () => {
  ...
  test("バリデーションエラーが正しく表示される", async ({ page }) => {
    ...
    expect(errorMessages.some(text => text.includes('メールアドレス') || text.includes('有効'))).toBeTruthy();
    expect(errorMessages.some(text => text.includes('お問い合わせ内容'))).toBeTruthy();
  });
  ...
  test("無効なメールアドレスを入力するとエラーが表示される", async ({ page }) => {
    ...
    await page.getByLabel("お問い合わせ内容").fill("これはテストメッセージです。これはテストメッセージです。");
    ...
    const hasVisibleEmailError = await page.evaluate(() => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const errorElements = parent.querySelectorAll('.text-destructive, .text-red-500, [role="alert"]');
    ...
    console.log('メールアドレスフィールド付近にエラーあり:', hasVisibleEmailError);
    console.log('メールアドレスフィールドは無効か:', isEmailInvalid);
    
    // いずれかの検証方法が成功すればテスト通過
    expect(hasVisibleEmailError || isEmailInvalid || formText.includes('有効なメールアドレス')).toBeTruthy();
  });
  ...
  test("電話連絡可の場合は電話番号が必須になる", async ({ page }) => {
    ...
    const hasFormErrors = formText.includes('ハイフン') || 
  ...
  test("電話連絡不可の場合は電話番号入力欄が表示されない", async ({ page }) => {
    ...
    await expect(page.locator('#phone-no')).toHaveAttribute('aria-checked', 'true');
    
    // 電話番号入力欄が表示されていないことを確認
    await expect(page.getByLabel("電話番号", { exact: true })).not.toBeVisible();
  });
  ...
  test("無効な電話番号形式を入力するとエラーが表示される", async ({ page }) => {
    ...
    const hasPhoneError = await page.evaluate(() => {
      const phoneInput = document.querySelector('input[placeholder*="電話番号"]');
    ...
    const formText = await page.locator('form').textContent() || '';
    const hasHyphenText = formText.includes('ハイフン') || formText.includes('-');
  ...
  test("有効なデータを送信すると成功メッセージが表示される", async ({ page }) => {
    // 必須項目を入力
    await page.getByLabel("お名前").fill("テスト 太郎");
    await page.getByLabel("メールアドレス").fill("test@example.com");
    await page.getByLabel("お問い合わせ内容").fill("これはテストメッセージです。これはテストメッセージです。");
    ...
    try {
      ...
      // 3. 緑色系のクラスを持つ要素
      const greenElements = await page.locator('.text-green-700, .bg-green-50, .text-green-800').allTextContents();
      console.log('緑色系要素のテキスト:', greenElements);
      
      // 少なくとも1つの成功メッセージが見つかればOK
      const hasSuccessMessage = 
        successText || 
        greenElements.some(text => text.includes('受け付け') || text.includes('営業日')) ||
  ...
  test("電話連絡可で有効なデータを送信すると成功メッセージが表示される", async ({ page }) => {
    // 必須項目を入力
    await page.getByLabel("お名前").fill("テスト 太郎");
    await page.getByLabel("メールアドレス").fill("test@example.com");
    await page.getByLabel("お問い合わせ内容").fill("これはテストメッセージです。これはテストメッセージです。");
    ...
    // ページ全体を確認
    const pageContent = await page.locator('body').textContent() || '';
    console.log('電話あり送信後のページ内容（一部）:', pageContent.substring(0, 300));
