...
class SecurityHeadersTestPage {
	...
	async getApiHeaders(path: string, method: 'GET' | 'POST' = 'GET', data?: any): Promise<{
	...
	// レート制限テスト用の複数リクエスト送信
	async sendMultipleRequests(path: string, count: number, delay: number = 0): Promise<Array<{
		status: number;
		headers: Record<string, string>;
		timestamp: number;
	}>> {
...
test.describe("HTTPセキュリティヘッダーテスト", () => {
	...
	test.describe("基本セキュリティヘッダー", () => {
		test("X-Content-Type-Options ヘッダーが正しく設定されている", async ({ page }) => {
		...
		test("X-XSS-Protection ヘッダーが正しく設定されている", async ({ page }) => {
			const headers = await securityPage.getHeadersForPath("/");
		...
		test("すべての主要ページでセキュリティヘッダーが設定されている", async ({ page }) => {
			const paths = ["/", "/about", "/services", "/contact", "/login", "/register"];
	...
	test.describe("Content Security Policy", () => {
		test("CSPヘッダーの存在確認（設定されている場合）", async ({ page }) => {
			...
			if (csp || cspReportOnly) {
				...
				console.log("CSP is not configured - this is optional but recommended for enhanced security");
			}
		});
		...
		test("meta CSPタグの存在確認（HTMLレベル）", async ({ page }) => {
			...
			if (cspMeta > 0) {
				...
				console.log("Meta CSP is not configured - using HTTP headers or no CSP");
			}
		});
	});
	...
	test.describe("レート制限機能", () => {
		test("API エンドポイントでのレート制限", async ({ page }) => {
			// 短時間で大量リクエストを送信
			const responses = await securityPage.sendMultipleRequests("/api/csrf-token", 25, 50);
		...
		test("ログインページでのレート制限", async ({ page }) => {
			const responses = await securityPage.sendMultipleRequests("/login", 25, 50);
			...
			if (rateLimitedCount > 0) {
				...
				console.log(`Rate limiting active: ${rateLimitedCount}/${responses.length} requests limited`);
			} else {
		...
		test("レート制限のリセット機能", async ({ page }) => {
			// 第1回目：レート制限まで送信
			const firstBatch = await securityPage.sendMultipleRequests("/api/csrf-token", 15, 50);
			const firstBatchLimited = firstBatch.filter(r => r.status === 429);
			
			// 十分な時間待機（レート制限ウィンドウのリセット）
			await page.waitForTimeout(11000); // 11秒待機（10秒のウィンドウ + バッファ）
			
			// 第2回目：リセット後のリクエスト
			const secondBatch = await securityPage.sendMultipleRequests("/api/csrf-token", 5);
	...
	test.describe("キャッシュ制御", () => {
			...
			for (const path of sensitivePages) {
				try {
					...
					if (cacheControl) {
						...
						if (hasNoCache) {
							console.log(`Appropriate cache control for ${path}: ${cacheControl}`);
						}
					}
		...
		test("静的リソースのキャッシュ最適化", async ({ page }) => {
			...
			const staticResources = responses.filter(r => {
				const url = r.url();
				return url.includes('/_next/static/') || url.endsWith('.css') || url.endsWith('.js');
			});
			
			for (const resource of staticResources.slice(0, 5)) { // 最初の5件のみテスト
				const headers = resource.headers();
				const cacheControl = headers["cache-control"];
				
				if (cacheControl) {
					// 静的リソースは長期キャッシュが効率的
					const hasLongCache = cacheControl.includes("public") || 
	...
	test.describe("エラーレスポンスのセキュリティ", () => {
		test("404エラー時の情報漏洩防止", async ({ page }) => {
			const { headers, status, body } = await securityPage.getApiHeaders("/api/nonexistent-endpoint");
			...
			expect(bodyString.toLowerCase()).not.toMatch(/password|secret|token|database|internal/);
		});

		test("500エラー時のスタックトレース非表示", async ({ page }) => {
			// 意図的にエラーを発生させる試行（存在しないAPIエンドポイント）
			const { status, body } = await securityPage.getApiHeaders("/api/cause-error", "POST", {
		...
		test("認証エラー時の適切なヘッダー", async ({ page }) => {
			...
			const { headers, status } = await securityPage.getApiHeaders("/dashboard");
	...
	test.describe("セキュリティヘッダーの一貫性", () => {
		test("全エンドポイントでセキュリティヘッダーが一貫している", async ({ page }) => {
			const endpoints = [
				"/",
				"/api/csrf-token",
				"/api/health", // 存在する場合
