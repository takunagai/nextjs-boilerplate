...
describe('サーバーアクションバリデーションユーティリティ', () => {
  describe('validateAction', () => {
    ...
    it('無効なデータの場合、ActionErrorをスローする', async () => {
      ...
      await expect(validateAction(testSchema, invalidData)).rejects.toMatchObject({
    ...
    it('カスタムエラーメッセージを設定できる', async () => {
      ...
      await expect(
        validateAction(testSchema, invalidData, { errorMessage: 'カスタムエラーメッセージ' })
    ...
    it('Zodエラー以外のエラーが発生した場合、UNKNOWN_VALIDATION_ERRORをスローする', async () => {
      ...
      await expect(validateAction(mockSchema as unknown as z.ZodType, {})).rejects.toMatchObject({
  ...
  describe('formatZodErrors', () => {
    it('Zodエラーを適切なフォーマットに変換する', () => {
      ...
      expect(formattedErrors.email).toBe('有効なメールアドレスを入力してください');
    ...
    it('複数のエラーがある場合は最後のエラーメッセージが使用される', () => {
      ...
      expect(formattedErrors.password).toBe('パスワードには数字を含める必要があります');
    });
  });
});
