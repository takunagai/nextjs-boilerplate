...
import { submitExampleForm, checkExampleEmail } from '@/app/actions/form-example';
...
describe('フォームサンプル用サーバーアクション', () => {
  ...
  describe('submitExampleForm', () => {
    ...
    it('有効なデータが送信された場合、成功レスポンスを返す', async () => {
      ...
      expect(result.data).toEqual({ message: 'フォームが正常に送信されました。' });
    ...
    it('バリデーションエラーが発生した場合、エラーレスポンスを返す', async () => {
      // バリデーションエラーをシミュレート
      const validationError = new ActionError('入力データが無効です', 'VALIDATION_ERROR', {
        fieldErrors: {
          name: '名前を入力してください',
        },
      });
