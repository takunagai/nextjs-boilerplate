						...
						<div key={item.id} className="relative overflow-hidden">
							...
							>
								...
								<div className="flex flex-col gap-6 max-w-xl [direction:ltr] relative z-20 order-2 md:order-none">
									...
									{/* 特徴リスト */}
									{(item as ServiceItem).features &&
										Array.isArray((item as ServiceItem).features) && (
											<ul className="space-y-3">
												{((item as ServiceItem).features as string[]).map(
													(feature: string) => (
														<li
															key={feature}
								...
								{item.imageUrl && (
									<div className="relative w-full h-full [direction:ltr] order-1 md:order-none">
										<div
											...
											style={{
												clipPath: `url(#blob-${(item as ServiceItem).blobShape})`,
											}}
										>
											<AnimatedImage
												src={item.imageUrl}
