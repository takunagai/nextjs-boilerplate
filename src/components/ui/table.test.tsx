...
describe("tableVariants", () => {
  ...
  it("should return correct classes for size 'lg'", () => {
    ...
    // lg base styles
    expect(classes).toContain("[&_th]:px-6 [&_td]:px-6 [&_th]:py-3 [&_td]:py-3 text-base");
    // lg container query styles
    expect(classes).toContain("@lg:[&_th]:px-4 @lg:[&_td]:px-4 @lg:[&_th]:py-2 @lg:[&_td]:py-2 @lg:text-sm");
    // md container query styles
    expect(classes).toContain("@md:[&_th]:px-2 @md:[&_td]:px-2 @md:[&_th]:py-1.5 @md:[&_td]:py-1.5 @md:text-xs");
    // sm container query styles
    expect(classes).toContain("@sm:[&_th]:px-1 @sm:[&_td]:px-1 @sm:[&_th]:py-0.5 @sm:[&_td]:py-0.5 @sm:text-xs");
  });
  ...
  it("should return correct classes for default size (md)", () => {
    ...
    expect(classes).toContain("[&_th]:px-4 [&_td]:px-4 [&_th]:py-2 [&_td]:py-2 text-sm");
  ...
  it("should return correct classes for size 'sm'", () => {
    ...
    expect(classes).toContain(baseClasses);
    // sm base styles
    expect(classes).toContain("[&_th]:px-2 [&_td]:px-2 [&_th]:py-1.5 [&_td]:py-1.5 text-xs");
    // sm container query styles
    expect(classes).toContain("@sm:[&_th]:px-1 @sm:[&_td]:px-1 @sm:[&_th]:py-0.5 @sm:[&_td]:py-0.5 @sm:text-xs");
  ...
  it("should return correct classes for size 'xs'", () => {
    ...
    expect(classes).toContain(baseClasses);
    // xs base styles
    expect(classes).toContain("[&_th]:px-1 [&_td]:px-1 [&_th]:py-0.5 [&_td]:py-0.5 text-xs");
