    ...
    >
      {items.map((item, i) => {
        ...
        return (
          <li
            key={i}
            className={clsx(liVariants({ inline }), itemClassName, item.className)}
            aria-label={item.ariaLabel}
          >
            {numberStyle === "badge" ? (
              <span className={badgeClass} aria-hidden="true">{number}</span>
            ) : (
              <span className="mr-2 font-mono text-gray-500 min-w-6 text-right tabular-nums" aria-hidden="true">{number}.</span>
            )}
            {isLink ? (
              <a
                href={item.url}
                ...
                tabIndex={0}
                aria-current={item.url && typeof window !== "undefined" && window.location?.pathname === item.url ? "page" : undefined}
              >
                {item.text}
              </a>
            ) : (
