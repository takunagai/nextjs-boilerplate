...
export function NewsPagination({
	...
	const showFirstPage =
		showEdges && !items.includes(firstPage) && firstPage < items[0];
	const showLastPage =
		showEdges &&
		!items.includes(lastPage) &&
		lastPage > items[items.length - 1];

	// 省略記号の表示制御
	const showStartEllipsis =
		showEllipsis && showFirstPage && items[0] > firstPage + 1;
	const showEndEllipsis =
		showEllipsis && showLastPage && items[items.length - 1] < lastPage - 1;
