...
export function MapCard({
  ...
  const mapSrc = iframeSrc || generateGoogleMapEmbedUrl({
    address,
    lat,
    lng,
    query,
  ...
  if (!mapSrc && !address && !(lat && lng) && !query) {
    console.error('MapCard: iframeSrc, address, lat&lng, または query のいずれかを指定してください');
  }
      ...
      <CardContent>
        ...
        <div className="space-y-4">
          <address className="not-italic">
            <Label htmlFor="map-address" className="block mb-1 text-sm font-medium">住所</Label>
            <p id="map-address" className="text-base">{address}</p>
          </address>
          
          {phone && (
            <div>
              <Label htmlFor="map-phone" className="block mb-1 text-sm font-medium">電話</Label>
              <a
                id="map-phone"
