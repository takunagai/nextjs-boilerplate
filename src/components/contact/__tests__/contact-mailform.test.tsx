...
describe('ContactEmailForm', () => {
  ...
  describe('Initial Rendering', () => {
    it('renders the form with initial fields and states', () => {
      ...
      expect(screen.getByText('電話連絡の可否 *', { selector: 'label' })).toBeInTheDocument();
  ...
  describe('Input Validation', () => {
    ...
    const messageMinLengthError = "お問い合わせ内容は10文字以上で入力してください";
    const phoneFormatError = "電話番号は「00-0000-0000」の形式で入力してください";
    ...
    it('shows error for message shorter than 10 characters and sets aria-invalid', async () => {
      ...
      await waitFor(() => expect(messageInput).toHaveAttribute('aria-invalid', 'true'));
      expect(await screen.findByText(messageMinLengthError)).toBeInTheDocument();
    });
  ...
  describe('Form Submission Logic', () => {
    ...
    it('successful submission resets form and shows success message', async () => {
      ...
      const buttonBeforeClick = screen.getByRole('button', { name: '送信する' });
      ...
      await waitFor(() => expect(mockSubmitContactForm).toHaveBeenCalledTimes(1));
    ...
    it('handles server-side validation error', async () => {
      ...
      await waitFor(() => expect(mockSubmitContactForm).toHaveBeenCalledTimes(1));
      expect(await screen.findByText('入力内容に問題があります。修正してください。')).toBeInTheDocument();
    ...
    it('handles server error with no specific message', async () => {
      ...
      await waitFor(() => expect(mockSubmitContactForm).toHaveBeenCalledTimes(1));
      expect(await screen.findByText('送信に失敗しました。後ほど再度お試しください。')).toBeInTheDocument();
    });
    ...
    it('shows loading state on button during submission', async () => {
      ...
      const buttonWhileSubmitting = await screen.findByRole('button', { name: '送信中...' });
      expect(buttonWhileSubmitting).toBeDisabled();
      
      await waitFor(() => 
        expect(screen.getByRole('button', { name: '送信する' })).not.toBeDisabled(), {timeout: 500}
      );
    });
  });
});
