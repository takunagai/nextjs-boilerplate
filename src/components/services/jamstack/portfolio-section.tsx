...
import { FaArrowUpRightFromSquare, FaRocket, FaMagnifyingGlass, FaMobile } from "react-icons/fa6";
...
const portfolioItems = [
	...
	},
	{
		...
		description: "クリエイティブなアニメーション×高速表示を両立したポートフォリオサイト",
	...
	{
		title: "スタートアップLP",
		description: "新サービスのランディングページ。SEO最適化で検索流入200%アップ",
							...
							<div className="p-6 space-y-4">
								...
								<div className="grid grid-cols-3 gap-4 p-3 bg-muted/50 rounded-lg">
									<div className="text-center">
										...
										<span className="font-bold text-sm">{item.metrics.performance}</span>
									</div>
									<div className="text-center">
										<div className="flex items-center justify-center mb-1">
										...
										<span className="font-bold text-sm">{item.metrics.seo}</span>
									</div>
									<div className="text-center">
										<div className="flex items-center justify-center mb-1">
											<FaMobile className="w-3 h-3 text-purple-600 mr-1" />
											<span className="text-xs text-muted-foreground">読込</span>
										</div>
										<span className="font-bold text-sm">{item.metrics.loadTime}</span>
									</div>
								</div>

								{/* 技術スタック */}
								<div>
									<h4 className="text-xs font-semibold text-muted-foreground mb-2">使用技術:</h4>
