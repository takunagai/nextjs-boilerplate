...
export function WhalesAnimation() {
  ...
  useEffect(() => {
    ...
    // 巨大な鯨を1-2匹のみ生成
    const whaleCount = Math.min(2, Math.max(1, Math.floor(dimensions.width / 1200)));
    ...
    for (let i = 0; i < whaleCount; i++) {
      ...
      newWhales.push({
        id: `whale-${i}`,
        initialX: direction === 1 ? -whaleSize * 0.5 : dimensions.width - whaleSize * 0.5, // 画面の外から少し見えるように
        initialY: dimensions.height * 0.1 + Math.random() * (dimensions.height * 0.3), // 上部にランダムに配置
      ...
      {whales.map((whale) => (
        ...
        >
          ...
          >
            ...
            />
            {/* 鯨の目 */}
            <circle cx="90" cy="50" r="5" fill="#0284c7" className="opacity-60 dark:opacity-40" />
            {/* 鯨の水噴射 */}
            <path
