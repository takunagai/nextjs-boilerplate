...
function ColumnLayout({
	...
	for (const child of childrenArray) {
		...
		const displayName =
			typeof component === "function" ? component.displayName : undefined;
	...
	return (
		<div
			className={cn(columnLayoutVariants({ variant }), gap, containerClassName)}
		>
...
function Left({ children, className }: ColumnProps) {
	return <div className={cn("w-full md:w-1/3", className)}>{children}</div>;
}
Left.displayName = "ColumnLayout.Left";

// メインカラムコンポーネント
function Main({ children, className }: ColumnProps) {
	return <div className={cn("w-full md:w-2/3", className)}>{children}</div>;
}
