...
function getCategoryFromTags(tags: string[]): "web" | "design" | "photo" | "logo" | "shop" | "other" {
...
export function getPortfolioItemsByTag(tag?: string): PortfolioItem[] {
  ...
  return portfolioItems.filter(item => item.servicesTags && item.servicesTags.includes(tag));
}
